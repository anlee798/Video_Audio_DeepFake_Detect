...
class Audio_RNN_ResNext(nn.Module):
    '''
    input:
        video : 1,1,3,16,224,224
        audio : 1, 1, 1, 13, 99
    '''
    def __init__(self, img_dim=224, num_layers_in_fc_layers=1024, dropout=0.5):
        super(Audio_RNN_ResNext, self).__init__();
        ...
        self.netcnnlip = resnext101()
        # 预训练模型加载
        pretrained_path = './weights/kinetics_resnext_101_RGB_16_best.pth'
        ...
        model_state_dict = self.netcnnlip.state_dict()
        for k, v in checkpoint.items():
            ...
            if name in model_state_dict:
                ...
                print("不存在", name)
        self.netcnnlip.load_state_dict(model_state_dict)
    ...
    def forward_lip(self, x):
        ...
        feature = self.netcnnlip(x) # (1, 256, 1, 7, 7)
        feature = feature.view(B, N, -1, self.last_size, self.last_size)  # # (4,1,256,7,7)
