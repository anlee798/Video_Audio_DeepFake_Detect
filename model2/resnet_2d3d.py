...
class ResNet2d3d_full(nn.Module):
    ...
    def forward(self, x): # (4,3,30,224,224)
        x = self.conv1(x) # (4,64,30,112,112)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x) # (4,64,30,56,56)

        x = self.layer1(x) # (4,64,30,56,56)
        x = self.layer2(x) # (4,128,30,28,28)
        x = self.layer3(x) # (4,256,15,14,14)
        x = self.layer4(x) # (4,256,8,7,7)
